...
void* mm_malloc(size_t size) {
  ...
  // If no suitable block found, request more memory from the heap.
  ...
  // If the selected block is larger than the requested size, split it.
  if(block_size - req_size >= MIN_BLOCK_SIZE) {
    // Set tags for the allocated block.
    ptr_free_block->size_and_tags = req_size | TAG_USED;
    ptr_free_block->size_and_tags |= preceding_block_use_tag;

    // Update size and tags for the remaining free block.
    size_t* header_ptr = (size_t*)UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    size_t* footer_ptr = (size_t*)UNSCALED_POINTER_ADD(ptr_free_block, block_size - WORD_SIZE);
    size_t new_tag = (block_size - req_size) | TAG_PRECEDING_USED;
    *header_ptr = new_tag;
    *footer_ptr = new_tag;
    block_info* remaining_block = (block_info*)UNSCALED_POINTER_ADD(ptr_free_block, req_size);
    
    // Insert the remaining free block into the free list.
    insert_free_block(remaining_block);
  } else { 
    // Mark the allocated block as used.
    ptr_free_block->size_and_tags |= TAG_USED;
    // Mark the next block as preceded by a used block.
...
void mm_free(void* ptr) {
  ...
  // Get a pointer to the header of the block being freed.
  ...
  // Clear the used tag in the header and set the footer to match the header.
  block_to_free->size_and_tags = (block_to_free -> size_and_tags) & (~TAG_USED); 
  size_t* footer_ptr = (size_t*)UNSCALED_POINTER_ADD(block_to_free, payload_size - WORD_SIZE);
  *footer_ptr = block_to_free->size_and_tags;
  // Clear the preceding used tag in the next block.
  following_block->size_and_tags= (following_block->size_and_tags) & (~TAG_PRECEDING_USED);
  // Coalesce the freed block with neighboring free blocks and insert it into the free list.
